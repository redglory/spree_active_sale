...
module Spree
  class SaleEvent < ActiveRecord::Base
    ...
    scope :live, -> { where("(start_date <= :start_date AND end_date >= :end_date) OR is_permanent = :is_permanent", { :start_date => zone_time, :end_date => zone_time, :is_permanent => true }) }
    scope :active, ->(*args) { where(:is_active => valid_argument(args)) }
    scope :hidden, ->(*args) { where(:is_hidden => valid_argument(args)) }
    scope :live_active, ->(*args) { self.live.active(valid_argument(args)) }
    scope :live_active_and_hidden, ->(*args) { 
    ...
    scope :upcoming_events, -> { where("start_date > :start_date", { :start_date => zone_time }) }
    scope :past_events, -> { where("end_date < :end_date", { :end_date => zone_time }) }
    scope :starting_today, -> { where(:start_date => zone_time..zone_time.end_of_day) }
    scope :ending_today, -> { where(:end_date => zone_time..zone_time.end_of_day) }
